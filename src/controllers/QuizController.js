...
export const initQuiz = async (playMode, quizId, battleId) => {
  ...
  let url =
    ...
    "/quizzito/playquiz/load_quiz_playground/";

  let header = await Request.getHeader(HTTPMethods.POST, formData);
  console.log("INIT QUIZ REQUEST ", playMode, quizId, battleId, url, header);
...
export const nextQuestion = async (answserID, quizId) => {
  ...
  let url =
    ...
    QuizzitoConstant.PRE_END_POINT +
    QuizzitoConstant.IS_MAIN_NEW +
    "/quizzito/playquiz/next_question2_custom";

  let header = await Request.getHeader(HTTPMethods.POST, formData);
  console.log("NEXT QEUSTION ", answserID, quizId, header, url);
  return fetch(url, header)
    .then((response) => Request.processResponse(response))
    .then((responseData) => {
